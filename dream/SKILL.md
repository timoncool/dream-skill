...
3. **Автостоп-лист** — валидатор НЕ имеет права одобрить, только юзер вручную:
   ...
   - action=`promote_skill` (активация скилла = инструкции будущим сессиям — высокое влияние)
...
- Автостоп-лист для **actions снимается**: валидатор может одобрять `delete`, `purge_trash` и `promote_skill`/`retire_skill`. Компенсации: wake в full auto делает pre-delete backup файла в TRASH перед очисткой контента; для promote_skill валидатор обязан пройти чек-лист драфта (Use when + триггер-фразы, injection, дубли — см. промпт п.2), а wake бэкапит перезаписываемое и логирует путь активации — «откатить скилл» = mv папки обратно в staging. Необратимого не остаётся вовсе.
