...
```text
Установи скиллы консолидации памяти dream-skill из
https://github.com/timoncool/dream-skill — склонируй репо во временную папку,
скопируй папки dream/ и wake/ в ~/.claude/skills/ (создай, если нет; если
попрошу установку в проект — в <проект>/.claude/skills/), проверь что оба
SKILL.md на месте вместе с references/ и assets/, покажи список установленных
файлов и напомни перезапустить Claude Code, чтобы скиллы подгрузились.
```

Всё — Клод склонирует, скопирует, проверит и отчитается. Ручной способ:
